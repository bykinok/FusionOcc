...
# -*- coding: utf-8 -*-
"""
surroundocc pkl 파일에 occfrmwrk pkl 파일의 occ_path 정보를 추가하는 스크립트

scene_token을 기준으로 매칭하여 occ_path를 추가합니다.

사용법:
    python tools/merge_occ_path_to_surroundocc.py
"""
...
def merge_occ_paths(surroundocc_file, occfrmwrk_file, output_file=None, key_name='occ3d_gt_path'):
    """
    surroundocc pkl 파일에 occfrmwrk pkl 파일의 occ_path 정보를 추가
    
    Args:
        surroundocc_file (str): SurroundOcc 형식의 pkl 파일 경로
        occfrmwrk_file (str): occfrmwrk 형식의 pkl 파일 경로 (occ_path 소스)
        output_file (str, optional): 출력 파일 경로. None이면 surroundocc_file을 덮어씀
    ...
    """
    # 파일 로드
    surroundocc = load_pkl(surroundocc_file)
    occfrmwrk = load_pkl(occfrmwrk_file)
    
    # data_list 추출 (두 형식 모두 지원)
    surroundocc_list = surroundocc.get('data_list', surroundocc.get('infos', []))
    occfrmwrk_list = occfrmwrk.get('data_list', occfrmwrk.get('infos', []))
    
    print(f"\nSurroundOcc items: {len(surroundocc_list)}")
    ...
    # SurroundOcc에 occ_path 추가
    print("\nMerging occ_path to surroundocc...")
    matched = 0
    not_matched = 0
    multiple_matches = 0
    
    for item in tqdm(surroundocc_list, desc="Merging"):
        scene_token = item.get('scene_token')
    ...
    print(f"{'='*60}")
    print(f"Total items:          {len(surroundocc_list)}")
    print(f"Matched:              {matched} ({matched/len(surroundocc_list)*100:.1f}%)")
    print(f"Not matched:          {not_matched} ({not_matched/len(surroundocc_list)*100:.1f}%)")
    ...
    if output_file is None:
        output_file = surroundocc_file
    
    save_pkl(surroundocc, output_file)
...
def main():
    ...
    files_to_process = [
        ('surroundocc-nuscenes_infos_train.pkl', 'occfrmwrk-nuscenes_infos_train.pkl'),
        ('surroundocc-nuscenes_infos_val.pkl', 'occfrmwrk-nuscenes_infos_val.pkl'),
    ]
    
    for surroundocc_file, occfrmwrk_file in files_to_process:
        surroundocc_path = osp.join(data_root, surroundocc_file)
        occfrmwrk_path = osp.join(data_root, occfrmwrk_file)
        
        print(f"\n{'#'*70}")
        print(f"# Processing: {surroundocc_file}")
        print(f"{'#'*70}")
        
        # 파일 존재 확인
        if not osp.exists(surroundocc_path):
            print(f"✗ Error: {surroundocc_path} not found!")
            continue
        ...
        # 백업 생성
        backup_path = surroundocc_path + '.backup'
        if not osp.exists(backup_path):
            print(f"Creating backup: {backup_path}")
            shutil.copy2(surroundocc_path, backup_path)
        else:
        ...
        try:
            ...
            stats = merge_occ_paths(
                ...
                output_file=surroundocc_path,
                key_name='occ3d_gt_path'
            )
            
            print(f"\n✓ Merge completed successfully!")
            print(f"  - Added key: 'occ3d_gt_path'")
            print(f"  - Matched: {stats['matched']}/{stats['total']}")
            
        except Exception as e:
            print(f"\n✗ Error processing {surroundocc_file}: {e}")
            import traceback
            traceback.print_exc()
            ...
            if osp.exists(backup_path):
                ...
                shutil.copy2(backup_path, surroundocc_path)
            continue
