...
class SurroundOccPointToMultiViewDepth(BaseTransform):
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}(downsample={self.downsample}, "
                f"grid_config={self.grid_config}, "
                f"selected_classes={list(self.selected_classes) if self.selected_classes is not None else None})")