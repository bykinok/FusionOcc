...
#
# surroundocc_occ3d_wo_train_cam_mask.py
#
# Based on surroundocc_occ3d_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_mask_camera_train=False → LoadOccupancy가 비가시 voxel을 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_image_mask=True (OccupancyMetricHybrid) → 카메라 가시 voxel만 mIoU 계산
#                 (mask_camera는 use_mask_camera와 무관하게 항상 results에 저장됨)
#
custom_imports = dict(
...
use_mask_camera = True         # 평가용 파이프라인: camera mask 유지
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         use_occ3d=True if dataset_name == 'occ3d' else False,
         pc_range=point_cloud_range if dataset_name == 'occ3d' else None,
         use_mask_camera=use_mask_camera,  # 평가용 파이프라인: camera mask 유지
         use_mask_camera_1_2=use_mask_camera_1_2,
         use_mask_camera_1_4=use_mask_camera_1_4,
         use_mask_camera_1_8=use_mask_camera_1_8),
    dict(type='NormalizeMultiviewImage', **img_norm_cfg),
...
val_evaluator = dict(
    ...
    use_image_mask=True if dataset_name == 'occ3d' else False,  # 평가 시 camera mask 사용 → 카메라 가시 voxel만 mIoU 계산
