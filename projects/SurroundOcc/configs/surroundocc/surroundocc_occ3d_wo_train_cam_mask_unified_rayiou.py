...
#
# surroundocc_occ3d_wo_train_cam_mask_unified_rayiou.py
#
# Based on surroundocc_occ3d_wo_train_cam_mask_unified.py
#
# Camera mask policy:
#   - Training  : use_mask_camera=False → LoadOccupancy가 비가시 voxel을 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_mask_camera_eval=False → RayIoU는 camera mask 불필요
...
# Mask camera option (학습/평가 분리)
use_mask_camera = False       # 학습 시 camera mask 미사용
use_mask_camera_eval = False  # 평가 시 camera mask 미사용 (RayIoU 기반)
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         use_occ3d=True if dataset_name == 'occ3d' else False,
         pc_range=point_cloud_range if dataset_name == 'occ3d' else None,
         use_mask_camera=use_mask_camera_eval,     # RayIoU: camera mask 미적용 (False)
         use_mask_camera_1_2=use_mask_camera_1_2,
         use_mask_camera_1_4=use_mask_camera_1_4,
         use_mask_camera_1_8=use_mask_camera_1_8),
