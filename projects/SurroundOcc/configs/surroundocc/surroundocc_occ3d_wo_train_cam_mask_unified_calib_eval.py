...
model = dict(
    ...
    temperature=1.9533,  # Temperature scaling (NLL-calibrated on val_calib) — update after tools/train_temperature.py
    img_backbone=dict(
       type='ResNet',
       depth=101,
       num_stages=4,
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='surroundocc-nuscenes_infos_val_eval.pkl',
...
val_evaluator = dict(
    ...
    use_image_mask=True if dataset_name == 'occ3d' else False,  # 평가 시 camera mask 사용
    ann_file='data/nuscenes/surroundocc-nuscenes_infos_val_eval.pkl',
    data_root='data/nuscenes',
