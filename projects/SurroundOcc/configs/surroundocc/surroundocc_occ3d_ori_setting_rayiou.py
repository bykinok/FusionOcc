...
#
# surroundocc_occ3d_ori_setting_rayiou.py
#
# Based on surroundocc_occ3d_ori_setting.py
#
# Evaluation metric: RayIoU (LiDAR ray 기반)
#   - Training  : use_mask_camera=True → LoadOccupancy가 비가시 voxel을 255로 마스킹
#   - Evaluation: use_mask_camera=False → GT voxel 전체 사용 (RayIoU는 camera mask 불필요)
...
# Mask camera option: 학습 시 적용, 평가(RayIoU) 시 미적용
use_mask_camera = True       # 학습용: camera mask 적용 (invisible voxels → 255)
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         use_occ3d=True if dataset_name == 'occ3d' else False,
         pc_range=point_cloud_range if dataset_name == 'occ3d' else None,
         use_mask_camera=False,  # RayIoU: camera mask 미적용 → GT voxel 전체 사용
         use_mask_camera_1_2=use_mask_camera_1_2,
         use_mask_camera_1_4=use_mask_camera_1_4,
         use_mask_camera_1_8=use_mask_camera_1_8),
    dict(type='NormalizeMultiviewImage', **img_norm_cfg),
