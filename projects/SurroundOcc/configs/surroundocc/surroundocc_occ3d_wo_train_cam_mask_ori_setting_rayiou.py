...
#
# surroundocc_occ3d_wo_train_cam_mask_ori_setting_rayiou.py
#
# Based on surroundocc_occ3d_wo_train_cam_mask_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_mask_camera_train=False → LoadOccupancy가 비가시 voxel을 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_mask_camera=False → RayIoU는 camera mask 불필요
...
use_mask_camera = False        # 평가용: RayIoU → camera mask 미적용
...
train_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         use_mask_camera=use_mask_camera_train,  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 계산
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         use_occ3d=True if dataset_name == 'occ3d' else False,
         pc_range=point_cloud_range if dataset_name == 'occ3d' else None,
         use_mask_camera=use_mask_camera,  # RayIoU: camera mask 미적용 → GT voxel 전체 사용
         use_mask_camera_1_2=use_mask_camera_1_2,
         use_mask_camera_1_4=use_mask_camera_1_4,
         use_mask_camera_1_8=use_mask_camera_1_8),
    dict(type='NormalizeMultiviewImage', **img_norm_cfg),
