...
# 구버전 mmdet.models.builder.{LOSSES, build_loss} → compat 경유
from ..compat import LOSSES, build_loss, reduce_mean
...
def compute_occ_loss(occ_pred, occ_target, mask=None, pos_weight=1.0, num_classes=18, mask_camera=None):
...
class Mask2FormerLoss(nn.Module):
    ...
    def forward(self, mask_pred, class_pred, mask_gt, class_gt, indices, mask_camera):
        ...
        for b in range(bs):
            ...
            src_mask = mask_pred[b][src_idx]
            tgt_mask = tgt_mask[tgt_idx]
            src_class = class_pred[b]
