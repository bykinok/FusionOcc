"""SparseOcc_unified r50 NuScenes 704x256 8frames — w/ train camera mask (mmengine 형식) — calib_train.

원본: projects/SparseOcc_eccv/configs/r50_nuimg_704x256_8f_w_cam_mask_unified_miou.py
변경 사항 (unified → calib_train):
  1. [val_dataloader] ann_file: nuscenes_infos_val_sweep.pkl
                             → nuscenes_infos_val_sweep_calib.pkl
  2. [val_evaluator]  ann_file: nuscenes_infos_val_sweep.pkl
                             → nuscenes_infos_val_sweep_calib.pkl
  (test_dataloader = val_dataloader 이므로 자동 반영)
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=dataset_root,
        occ_gt_root=occ_gt_root,
        ann_file=dataset_root + 'nuscenes_infos_val_sweep_calib.pkl',  # [변경] calib pkl 사용
