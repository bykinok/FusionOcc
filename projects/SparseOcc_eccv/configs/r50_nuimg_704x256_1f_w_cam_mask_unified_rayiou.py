"""SparseOcc_unified r50 NuScenes 704x256 1frame — w/ train camera mask (mmengine 형식) — temporal ablation, RayIoU 평가.

원본: projects/SparseOcc_eccv/configs/r50_nuimg_704x256_8f_w_cam_mask_unified_rayiou.py
변경 사항 (8f → 1f, temporal ablation):
  ...
  2. [pipeline/LoadMultiViewImageFromMultiSweeps] sweeps_num=_num_frames_-1 → 0
     (자동 적용 — 별도 수정 불필요)
  3. [work_dir] sparseocc_eccv_r50_256x704_1f_w_cam_mask_unified_rayiou
...
Camera mask policy:
  ...
  - Evaluation: use_image_mask=True  → RayIoU도 카메라 가시 voxel에서만 계산
"""
...
model = dict(
    ...
    use_grid_mask=False,
    use_mask_camera=True,  # 학습 시 camera mask 적용 → 가시 voxel에서만 loss 계산
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
...
train_pipeline = [
    ...
    dict(type='LoadOccGTFromFile', num_classes=len(occ_class_names),
         load_mask_camera=True),  # mask_camera 로드
...
log_level = 'INFO'
work_dir = 'work_dirs/sparseocc_eccv_r50_256x704_1f_w_cam_mask_unified_rayiou'
