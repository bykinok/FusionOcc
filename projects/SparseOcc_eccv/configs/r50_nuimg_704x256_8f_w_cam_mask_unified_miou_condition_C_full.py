"""SparseOcc_eccv r50 704x256 8f — condition_C_full (occupied full-range supervision).

Base: r50_nuimg_704x256_8f_w_cam_mask_unified_miou.py
변경 사항:
  1. [pipeline/LoadOccGTFromFile] mask_mode='condition_C_full'
     → ALL occupied voxels에 대해 mask_camera=1 강제 (항상 supervised)
     → invisible free voxels는 기존 mask_camera 유지 (supervised 안 됨)
  2. [work_dir] sparseocc_eccv_r50_256x704_8f_w_cam_mask_unified_miou_condition_C_full

Comparison (occupied voxel invisible supervision):
  condition_C_20m  → occupied 0–20 m invisible + all visible supervised
  condition_C      → occupied 0–35 m invisible + all visible supervised
  condition_C_full → occupied 0–∞  invisible + all visible supervised (this config)

Camera mask policy:
  - Training  : condition_C_full → ALL occupied voxels mask_camera=1 강제
  - Evaluation: use_image_mask=True → mIoU는 카메라 가시 voxel에서만 계산
"""
...
# condition_C_full: ALL occupied voxels force mask_camera=1 (always supervised);
...
# Eval pipeline is unchanged (OccupancyMetricHybrid loads mask_camera independently)
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles', to_float32=False, color_type='color'),
...
log_level = 'INFO'
work_dir = 'work_dirs/sparseocc_eccv_r50_256x704_8f_w_cam_mask_unified_miou_condition_C_full'
