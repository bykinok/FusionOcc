"""SparseOcc_eccv r50 NuScenes 704x256 8frames — w/ train camera mask — calib_eval export.

원본: r50_nuimg_704x256_8f_w_cam_mask_unified_calib_train_miou.py
...
변경 사항 (calib_test_T → calib_eval export):
  ...
  4. [val_dataloader] nuscenes_infos_val_sweep_calib.pkl → nuscenes_infos_val_sweep_eval.pkl
     (calib_train split → calib_eval split 으로 수정)

사용 흐름:
  # Uncalibrated logit 추출 (Step 3)
  python tools/export_occ_logits.py \\
      projects/SparseOcc_eccv/configs/r50_nuimg_704x256_8f_w_cam_mask_unified_calib_test_T_miou.py \\
...
model = dict(
    ...
    use_grid_mask=False,
    use_mask_camera=True,  # 학습 시 camera mask 적용
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=dataset_root,
        occ_gt_root=occ_gt_root,
        ann_file=dataset_root + 'nuscenes_infos_val_sweep_eval.pkl',  # calib_eval split
...
log_level = 'INFO'
work_dir = 'work_dirs/sparseocc_eccv_r50_256x704_8f_w_cam_mask_unified_calib_eval'
