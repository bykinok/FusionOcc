"""SparseOcc_eccv r50 704x256 8f — condition_D_full (free full-range supervision).

Base: r50_nuimg_704x256_8f_w_cam_mask_unified_miou.py
변경 사항:
  1. [pipeline/LoadOccGTFromFile] mask_mode='condition_D_full'
     → ALL free voxels에 대해 mask_camera=1 강제 (항상 supervised)
     → invisible occupied voxels는 기존 mask_camera 유지 (supervised 안 됨)
  2. [work_dir] sparseocc_eccv_r50_256x704_8f_w_cam_mask_unified_miou_condition_D_full

Comparison (free voxel invisible supervision):
  condition_D       → free 0–20 m invisible + all visible supervised
  condition_D_prime → free 0–35 m invisible + all visible supervised
  condition_D_full  → free 0–∞  invisible + all visible supervised (this config)

Camera mask policy:
  - Training  : condition_D_full → ALL free voxels mask_camera=1 강제
...
log_level = 'INFO'
work_dir = 'work_dirs/sparseocc_eccv_r50_256x704_8f_w_cam_mask_unified_miou_condition_D_full'
