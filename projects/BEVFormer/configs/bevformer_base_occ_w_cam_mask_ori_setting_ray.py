# Ray-aligned auxiliary loss variant of bevformer_base_occ_w_cam_mask_ori_setting.
#
# Difference from base config:
#   pts_bbox_head.ray_aux_loss is added with RayAlignedLoss (v4).
#
# V4 changes over V3:
#   1. Loss design: depth-ordered BCE + first-hit CE  (no transmittance chain)
#   2. Delayed activation: ray_loss_start_epoch / ray_loss_warmup_epochs
#   3. Per-sample normalization: normalize_per_sample=True (stable scale)
#   4. RayLossEpochHook registered automatically — added to custom_hooks below
#
# To DISABLE the ray loss for ablation: remove / comment out the
#   `ray_aux_loss=dict(...)` block inside pts_bbox_head.
#   The rest of the training pipeline is identical to the base config.
#
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        loss_occ=dict(
            ...
            use_sigmoid=False,
            loss_weight=1.0),
        # ── Ray-aligned auxiliary loss (v4: depth-ordered BCE + first-hit CE) ─
        # Replaces the V3 transmittance-NLL design that caused mIoU collapse.
        #
        # To disable entirely for ablation: comment out this ray_aux_loss block.
        #
        # pc_range / voxel_size / grid_size: must match the occ grid.
        # lidar_origin: FALLBACK only — per-sample origin is read from
        #   img_metas['ego2lidar'] at training time.
        # num_rays_train: subsample from ~15 840 total rays to cap memory.
        # use_mask: restrict valid rays to camera-visible first-hit voxels.
        #
        # ray_loss_mode options:
        #   "ordered_bce"       – (DEFAULT) depth-ordered BCE + first-hit CE
        #   "transmittance_nll" – legacy V3 NeRF-style NLL (ablation only)
        #
        # Component weights (ordered_bce mode only):
        #   w_bce_pre   – weight for per-step pre-hit free-space BCE
        #   w_bce_hit   – weight for per-ray  first-hit occupancy BCE
        #   w_ce_hit    – weight for per-ray  first-hit semantic CE
        #
        # loss_weight: outer scalar applied to the combined ray loss.
        #   Start at 0.1 and tune based on ratio loss_ray / loss_occ in logs.
        #   A ratio of 0.1–0.3 is recommended; >1.0 risks suppressing semantics.
        ray_aux_loss=dict(
            type='mmdet3d.RayAlignedLoss',
            pc_range=point_cloud_range,
            voxel_size=0.4,
            grid_size=[200, 200, 16],
            lidar_origin=[0.9858, 0.0, 1.8402],  # fallback; overridden per-sample
            free_class_id=17,
            loss_weight=0.1,
            eps=1e-6,
            num_rays_train=2000,
            ...
            use_mask=True,
            # Loss mode ──────────────────────────────────────────────────────
            ray_loss_mode='ordered_bce',  # "ordered_bce" | "transmittance_nll"
            w_bce_pre=1.0,               # pre-hit free-space BCE per-step weight
            w_bce_hit=1.0,               # first-hit occupancy BCE weight
            w_ce_hit=1.0,                # first-hit semantic CE weight
            # Delayed activation & warmup ────────────────────────────────────
            # ray loss is zero before epoch ray_loss_start_epoch,
            # then linearly ramps over ray_loss_warmup_epochs.
            # Both set to 0: activate immediately from epoch 0 (feasibility check).
            # For stable long runs restore: start_epoch=1, warmup_epochs=2
            ray_loss_start_epoch=0,      # activate from epoch 0
            ray_loss_warmup_epochs=0,    # no ramp: step activation
            # Per-sample normalization ───────────────────────────────────────
            # True (default): each sample contributes equally; stable scale.
            # False: global mean over all valid elements (original behaviour).
            normalize_per_sample=True,
            # Debug diagnostics ──────────────────────────────────────────────
            # When True, adds 5 extra scalar keys to the log every iteration:
            #   dbg_p_occ_pre  : mean p_occ on pre-hit voxels      → should ↓ to 0
            #   dbg_p_occ_hit  : mean p_occ at first-hit voxels    → should ↑ to 1
            #   dbg_no_hit_frac: fraction of rays with no valid hit → high = sparse scene
            #   dbg_hit_depth  : mean step index of first-hit voxels
            #   dbg_hit_class  : mean GT semantic class at first-hit voxels (< 17 expected)
            # Set to False for main experiments to avoid unnecessary overhead.
            ray_loss_debug=False,
        ),
        transformer=dict(
...
# ── RayLossEpochHook ────────────────────────────────────────────────────────
# Calls pts_bbox_head.set_epoch(runner.epoch) before each training epoch so
# that RayAlignedLoss can apply the correct delayed-activation / warmup scale.
# The hook class is registered automatically when ray_aligned_loss.py is
# imported (which happens when the model is built from config).
custom_hooks = [
    dict(type='RayLossEpochHook', priority='NORMAL'),
]
