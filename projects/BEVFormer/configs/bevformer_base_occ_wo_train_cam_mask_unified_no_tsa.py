# Temporal Self Attention (TSA) 비활성화 버전
# 원본: bevformer_base_occ_wo_train_cam_mask_unified.py
# 변경사항:
#   - TemporalSelfAttention 제거 (attn_cfgs에서 삭제)
#   - operation_order에서 'self_attn', 'norm' 제거
#   - video_test_mode=False (이전 BEV 히스토리 미사용)
#   - rotate_prev_bev=False, use_shift=False, use_can_bus=False
#   - queue_length=1 (현재 프레임만 사용)
...
bev_h_ = 200
bev_w_ = 200
queue_length = 1  # TSA 비활성화: 현재 프레임만 사용 (원본: 4)

model = dict(
    type='mmdet3d.BEVFormerOcc',
    use_grid_mask=True,
    video_test_mode=False,  # TSA 비활성화: 이전 BEV 히스토리 미사용 (원본: True)
    ...
    pts_bbox_head=dict(
        ...
        as_two_stage=False,
        use_mask=False,  # 학습 시 camera mask 미사용
        loss_occ= dict(
        ...
        transformer=dict(
            ...
            use_3d=True,
            use_conv=False,
            rotate_prev_bev=False,   # TSA 비활성화: 이전 BEV 회전 미사용 (원본: True)
            use_shift=False,         # TSA 비활성화: BEV shift 미사용 (원본: True)
            use_can_bus=False,       # TSA 비활성화: can bus 정보 미사용 (원본: True)
            embed_dims=_dim_,
            encoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        # TSA 비활성화: TemporalSelfAttention 제거
                        # dict(
                    ...
                    ffn_dropout=0.1,
                    # TSA 비활성화: 'self_attn', 'norm' 제거 (원본: ('self_attn', 'norm', 'cross_attn', 'norm', 'ffn', 'norm'))
...
# 학습 시 camera mask 미사용: mask_camera를 collect하지 않음
train_pipeline = [
    ...
    dict(type='CustomCollect3D', keys=['img', 'voxel_semantics', 'mask_lidar'])  # mask_camera 제거
]

# 평가 시 camera mask 사용: mask_camera 유지
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles', to_float32=True),
...
# 평가 시 camera mask 사용 (use_image_mask=True 유지)
val_evaluator = dict(
    ...
    use_image_mask=True,  # 평가 시 camera mask 사용
...
# find_unused_parameters=True: TSA 제거 후에도 일부 파라미터가 gradient를 받지 못할 수 있음
