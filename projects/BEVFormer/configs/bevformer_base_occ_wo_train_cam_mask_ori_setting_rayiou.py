# bevformer_base_occ_wo_train_cam_mask_ori_setting.py
#
# Based on bevformer_base_occ_w_cam_mask_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_mask=False  → loss is computed over ALL voxels (no mask applied)
#   - Evaluation: use_image_mask=False → RayIoU without camera mask (LiDAR ray-based)
#
# RayIoU variant of bevformer_base_occ_wo_train_cam_mask_ori_setting.
...
dataset_name = 'occ3d'
eval_metric = 'rayiou'  # rayiou without camera mask
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        as_two_stage=False,
        use_mask=False,  # 학습 시 camera mask 미사용 → 모든 voxel에 대해 loss 계산
        loss_occ= dict(
...
# mask_camera는 train_pipeline에도 수집해야 함:
#   - 학습 loss에는 사용하지 않지만 (use_mask=False)
#   - 평가 단계에서 OccupancyMetricHybrid가 data_sample에서 mask_camera를 읽기 때문
train_pipeline = [
...
# Enable validation and testing with OccupancyMetricHybrid
# CRITICAL: Use DistributedSampler for validation/test
val_dataloader = dict(
    batch_size=data['samples_per_gpu'],
    num_workers=data['workers_per_gpu'],
    persistent_workers=True,
...
# 평가 시 camera mask 미사용 → RayIoU (LiDAR ray 기반)
val_evaluator = dict(
    ...
    sort_by_timestamp=True,  # BEVFormer dataset sorts by timestamp
    prefix='val'
)
...
train_cfg = dict(
    ...
    val_interval=999999  # No validation during training
)
