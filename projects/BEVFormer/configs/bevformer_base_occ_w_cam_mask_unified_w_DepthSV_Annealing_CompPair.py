...
custom_imports = dict(
    imports=[
        'projects.BEVFormer',
        'projects.BEVFormer.datasets.samplers',
    ],
...
# Auxiliary depth supervision (ego-frame, same range as TPVFormer)
...
train_pipeline = [
    ...
    dict(type='CustomCollect3D', keys=['img', 'voxel_semantics', 'mask_lidar', 'mask_camera', 'gt_depth'])
]
...
# DepthLossAnnealingHook: depth loss_weight를 epoch에 따라 동적으로 조정
#   - DepthLossAnnealingHook (custom_hooks) overrides it before training
#   - Schedule is defined by depth_loss_annealing_schedule defined above.
custom_hooks = [
    dict(
