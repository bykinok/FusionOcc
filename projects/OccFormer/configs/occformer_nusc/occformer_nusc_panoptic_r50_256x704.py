...
stuff_classes = ['empty', 'driveable_surface', 'other_flat', 'sidewalk', 'terrain', 'manmade', 'vegetation']
thing_classes = ['barrier', 'bicycle', 'bus', 'car', 'construction_vehicle', 'motorcycle', 'pedestrian', 'traffic_cone', 'trailer', 'truck']

stuff_indices = [class_names.index(x) for x in stuff_classes]
thing_indices = [class_names.index(x) for x in thing_classes]
...
model = dict(
    ...
    pts_bbox_head=dict(
        type='Mask2FormerNuscPanopticOccHead',
        ...
        stuff_indices=stuff_indices,
        thing_indices=thing_indices,
...
data_root = '/mnt/cfs2/algorithm/yunpeng.zhang/nuscenes'
nusc_class_metas = 'projects/OccFormer/configs/_base_/nuscenes.yaml'
...
train_pipeline = [
    ...
    dict(type='LoadNuscPanopticOccupancyAnnotations', is_train=True, grid_size=occ_size, 
...
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles_OccFormer', is_train=False,
            data_config=data_config, img_norm_cfg=img_norm_cfg),
    dict(type='LoadNuscPanopticOccupancyAnnotations', is_train=False, grid_size=occ_size,
            point_cloud_range=point_cloud_range, bda_aug_conf=bda_aug_conf,
            cls_metas=nusc_class_metas),
    dict(type='OccDefaultFormatBundle3D', class_names=class_names, with_label=False), 
    dict(type='Collect3D', keys=['img_inputs', 'gt_occ', 'points_occ'],
            meta_keys=['pc_range', 'occ_size', 'sample_idx', 
                       'scene_token', 'img_filenames', 'scene_names']),
]
