# MMEngine 2.x Style Configuration - Cam + Lidar with Occ3D GT (RayIoU Evaluation)
#
# ssc_rs_base_nuscenes_cam_lidar_occ3d_wo_train_cam_mask_unified_rayiou.py
# Based on ssc_rs_base_nuscenes_cam_lidar_occ3d_wo_train_cam_mask_unified.py
#
# Camera mask policy:
#   - Training  : use_mask_camera=False → GT 마스킹 없이 전체 voxel에 loss 계산
#   - Evaluation: use_image_mask=False  → RayIoU는 ray casting으로 가시성 처리, camera mask 미사용
#                 use_mask_camera_eval=False → val/test 데이터셋에서 GT에 마스킹 없이 원본 voxel label 유지
#
work_dir = './work_dirs/ssc_rs_base_nuscenes_cam_lidar_occ3d_wo_train_cam_mask_unified_rayiou'
...
model = dict(
    ...
    img_bev_encoder_backbone=dict(
        ...
        temporal_adapter=False),
    # Lidar branch (cam_lidar: no radar)
    ...
    pts_backbone=dict(
        type='SemanticBranch',
        sizes=sizes,
        nbr_class=nbr_classes - 1,  # 17 semantic (exclude 'free')
        init_size=sizes[-1],
...
use_mask_camera = False       # 학습 시 camera mask 미사용
...
# 학습 시 camera mask 미사용 (use_mask_camera=False)
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        use_ego_frame=True,
        classes=class_names,
        use_mask_camera=use_mask_camera,        # 학습 시 camera mask 미사용 (False)
...
# 평가 시 camera mask 사용 (use_mask_camera_eval=True)
val_dataloader = dict(
...
# RayIoU: camera mask 미사용 (use_image_mask=False)
val_evaluator = [
    dict(
        ...
        use_image_mask=False,  # RayIoU: ray casting으로 가시성 처리, camera mask 미사용
...
load_from = './projects/LiCROcc/pre_ckpt/bevdet-r50-4d-depth-cbgs.pth'
resume = False
...
# Reproducibility: seed=0. EMA 미사용. SyncBN/AMP 검토 완료(모델 BN, OptimWrapper).
