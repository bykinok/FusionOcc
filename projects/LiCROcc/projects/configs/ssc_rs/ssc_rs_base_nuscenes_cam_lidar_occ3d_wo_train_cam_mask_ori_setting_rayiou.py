...
#
# ssc_rs_base_nuscenes_cam_lidar_occ3d_wo_train_cam_mask_ori_setting_rayiou.py
#
# Based on ssc_rs_base_nuscenes_cam_lidar_occ3d_wo_train_cam_mask_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_mask_camera_train=False → train_dataloader.dataset.use_mask_camera=False
#                 → 전체 voxel에 loss 계산
#   - Evaluation: use_image_mask=False → RayIoU는 ray casting으로 가시성 처리, camera mask 미사용
#                 use_mask_camera=False → val/test 데이터셋에서 GT에 마스킹 없이 원본 voxel label 유지
#
work_dir = './work_dirs/ssc_rs_base_nuscenes_cam_lidar_occ3d_wo_train_cam_mask_ori_setting_rayiou'
...
# Camera mask policy (학습/평가 분리)
use_mask_camera_train = False  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 계산
use_mask_camera = False        # RayIoU 평가: GT 마스킹 없이 원본 voxel label 유지
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        use_ego_frame=True,
        classes=class_names,
        use_mask_camera=use_mask_camera_train,  # 학습 시 camera mask 미사용
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        use_ego_frame=True,
        classes=class_names,
        use_mask_camera=use_mask_camera,  # RayIoU: GT 마스킹 없이 원본 voxel label 유지 (False)
        use_mask_camera_1_2=use_mask_camera_1_2,
        use_mask_camera_1_4=use_mask_camera_1_4,
        use_mask_camera_1_8=use_mask_camera_1_8,
    ),
