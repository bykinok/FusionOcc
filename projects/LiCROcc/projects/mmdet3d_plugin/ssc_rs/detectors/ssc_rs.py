# Compatibility layer for mmcv -> mmengine
from ...compat import auto_fp16, force_fp32

# Registry imports - use mmdet3d.registry.MODELS directly
from mmdet3d.registry import MODELS as DETECTORS
from mmdet3d.registry import MODELS  # For building components

print(f"[DEBUG ssc_rs.py] Importing SSC_RS, DETECTORS registry: {DETECTORS}")
print(f"[DEBUG ssc_rs.py] DETECTORS registry name: {DETECTORS.name}")
...
print(f"[DEBUG ssc_rs.py] About to register SSC_RS class to {DETECTORS.name} registry")

@DETECTORS.register_module(name='SSC_RS', force=True)
class SSC_RS(MVXTwoStageDetector):
    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__(**kwargs)
        print(f"[DEBUG ssc_rs.py] SSC_RS subclass initialized: {cls.__name__}")
    def __init__(self,
                use_grid_mask=False,
        ...
        if img_backbone_distill:
            self.img_backbone_distill = MODELS.build(img_backbone_distill)
            self.img_neck_distill = MODELS.build(img_neck_distill)
            self.img_view_transformer_distill = MODELS.build(img_view_transformer_distill)
            self.img_bev_encoder_backbone_distill = MODELS.build(img_bev_encoder_backbone_distill)
    ...
    def forward(self, inputs=None, data_samples=None, mode='tensor', return_loss=None, **kwargs):
        """
        MMEngine 2.x style forward method.
        Supports both old style (return_loss) and new style (mode).
        Note this setting will change the expected inputs. When
        `return_loss=True` or `mode='loss'`, img and img_metas are single-nested (i.e.
        torch.Tensor and list[dict]), and when `resturn_loss=False` or `mode='predict'`, 
        img and img_metas should be double nested (i.e.  list[torch.Tensor],
        list[list[dict]]), with the outer list indicating test time augmentations.
        """
        # Handle legacy return_loss parameter for backward compatibility
        if return_loss is not None:
            mode = 'loss' if return_loss else 'predict'
        
        # MMEngine 2.x style: use mode parameter
        if mode == 'loss':
            # Training mode
            ...
            raise ValueError(f"Invalid mode: {mode}. Must be 'loss', 'predict', or 'tensor'")
...
# Verify registration after class definition
print(f"[DEBUG ssc_rs.py] After class definition, checking registry...")
print(f"[DEBUG ssc_rs.py] SSC_RS in DETECTORS: {'SSC_RS' in DETECTORS.module_dict}")
if 'SSC_RS' in DETECTORS.module_dict:
    print(f"[DEBUG ssc_rs.py] SSC_RS registered as: {DETECTORS.module_dict['SSC_RS']}")
else:
    print(f"[DEBUG ssc_rs.py] Available models: {list(DETECTORS.module_dict.keys())[:10]}")

# Double-check: Ensure SSC_RS is also in the actual mmdet3d.registry.MODELS
from mmdet3d.registry import MODELS
print(f"[DEBUG ssc_rs.py] Checking actual mmdet3d.registry.MODELS...")
print(f"[DEBUG ssc_rs.py] MODELS is DETECTORS: {MODELS is DETECTORS}")
print(f"[DEBUG ssc_rs.py] SSC_RS in MODELS: {'SSC_RS' in MODELS.module_dict}")

# CRITICAL: Also register in mmengine.registry.MODELS (which Runner actually uses!)
from mmengine.registry import MODELS as MMENGINE_MODELS
print(f"[DEBUG ssc_rs.py] Checking mmengine.registry.MODELS...")
print(f"[DEBUG ssc_rs.py] SSC_RS in MMENGINE_MODELS: {'SSC_RS' in MMENGINE_MODELS.module_dict}")

if 'SSC_RS' not in MMENGINE_MODELS.module_dict:
    print(f"[DEBUG ssc_rs.py] Registering SSC_RS to mmengine.registry.MODELS...")
    MMENGINE_MODELS.register_module(name='SSC_RS', force=True, module=SSC_RS)
    print(f"[DEBUG ssc_rs.py] Registration complete! SSC_RS in MMENGINE_MODELS: {'SSC_RS' in MMENGINE_MODELS.module_dict}")
