...
class CrossModalLSS(LSSViewTransformerBEVDepth):
    ...
    def forward(self, input, sparse_depth):
        ...
        if depth_label is not None:
            ...
            # Create dummy depth input when no ground truth is available
            # depth_encoder expects depth_channels (88) input channels
            depth_input = torch.zeros(B * N, self.depth_channels, H, W, device=x.device, dtype=x.dtype)
