...
def force_fp32(apply_to=None, out_fp16=False):
    """Decorator to convert inputs to fp32 in forward."""
    def decorator(func):
        def wrapper(*args, **kwargs):
            if apply_to:
                new_args = []
                for i, arg in enumerate(args):
                    if i in apply_to or (isinstance(apply_to, (list, tuple)) and str(i) in [str(x) for x in apply_to]):
...
class LSSViewTransformer(BaseModule):
    ...
    def voxel_pooling_prepare_v2(self, coor):
        ...
        batch_idx = torch.arange(0, B, dtype=torch.float32, device=coor.device).reshape(B, 1). \
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        vis_gt_depth = gt_depths.clone().detach().long()
        gt_depths = F.one_hot(
            gt_depths.long(), num_classes=self.D + 1)

        gt_depths = gt_depths.view(-1, self.D + 1)[:, 1:]

        return gt_depths.float(), vis_gt_depth
