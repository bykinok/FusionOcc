...
class FPN_LSS(nn.Module):
    """FPN neck for LSS (Lift, Splat, Shoot).
    
    Args:
        in_channels (int): Number of input channels.
        out_channels (int): Number of output channels.
        scale_factor (int): Upsampling scale factor.
        input_feature_index (tuple): Indices of input features to use.
        norm_cfg (dict): Config for normalization layer.
        extra_upsample (int, optional): Extra upsampling scale factor.
        lateral (int, optional): Number of channels for lateral connection.
        use_input_conv (bool): Whether to use input conv.
    """
...
class LSSFPN3D(nn.Module):
    """3D FPN neck for LSS-based occupancy prediction.
    
    This neck upsamples and concatenates multi-scale 3D features.
    
    Args:
        in_channels (int): Total number of input channels (sum of all scales).
        out_channels (int): Number of output channels.
