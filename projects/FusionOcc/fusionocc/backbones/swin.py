...
from torch.nn.init import trunc_normal_ as trunc_normal_init, constant_ as constant_init
...
# Create ATTENTION registry for compatibility
ATTENTION = Registry('attention', locations=['projects.FusionOcc.fusionocc.backbones.swin'])

# Compatibility wrapper for resize
def resize(input, size=None, scale_factor=None, mode='nearest', align_corners=None):
    """Wrapper for F.interpolate to match mmseg.ops.resize interface."""
    ...
    def init_weights(self):
        if self.pretrained is None:
            ...
            ckpt = load_checkpoint(
                self, self.pretrained, logger=logger, map_location='cpu', strict=False)
