...
use_mask_eval = True    # 평가 시 camera mask 사용
...
model = dict(
    ...
    temperature=1.8861,  # Temperature scaling (NLL-calibrated on val_calib) — update after tools/train_temperature.py
...
test_data_config = dict(
    ...
    ann_file='data/nuscenes/fusionocc-nuscenes_infos_val_eval.pkl')
...
# 평가 시 camera mask 사용 (use_image_mask=use_mask_eval=True)
val_evaluator = dict(
    type='OccupancyMetricHybrid',
    num_classes=num_classes,
    use_lidar_mask=False,
    use_image_mask=use_mask_eval,  # 평가 시 camera mask 사용
    data_root='data/nuscenes/',
    ann_file='data/nuscenes/fusionocc-nuscenes_infos_val_eval.pkl',
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        ann_file='data/nuscenes/fusionocc-nuscenes_infos_val_eval.pkl',
        pipeline=test_pipeline,
        use_mask=use_mask_eval,  # 평가 시 camera mask 사용
        classes=class_names,
        modality=input_modality,
        stereo=False,
        filter_empty_gt=False,
...
work_dir = './work_dirs/fusion_occ_occ3d_miou_wo_train_cam_mask_unified'
