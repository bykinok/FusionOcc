...
#
# fusion_occ_occ3d_miou_wo_train_cam_mask.py
#
# Based on fusion_occ_occ3d_miou_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_mask_train=False → model.use_mask=False → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_mask_eval=True   → evaluator.use_image_mask=True → 카메라 가시 voxel만 mIoU 계산
#
...
use_mask_eval = True    # 평가 시 camera mask 사용 → 카메라 가시 voxel만 mIoU 계산
...
val_evaluator = dict(
    ...
    use_image_mask=use_mask_eval,  # 평가 시 camera mask 사용 → 카메라 가시 voxel만 mIoU 계산
    data_root='data/nuscenes/',
    ann_file='data/nuscenes/fusionocc-nuscenes_infos_val.pkl',
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        ann_file='data/nuscenes/fusionocc-nuscenes_infos_val.pkl',
        pipeline=test_pipeline,
        use_mask=use_mask_eval,  # 평가용 데이터셋: camera mask 사용
        classes=class_names,
        modality=input_modality,
        stereo=False,
        filter_empty_gt=False,
...
work_dir = './work_dirs/fusion_occ_occ3d_miou_wo_train_cam_mask'
