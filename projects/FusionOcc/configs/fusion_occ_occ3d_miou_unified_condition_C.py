...
# -----------------------------------------------------------------------
# Ablation: condition_C (occupied-only distance mask)
#   - Occupied voxels < 35 m from ego: always supervised (force mask_camera=1)
#     regardless of camera visibility.
#   - Occupied voxels >= 35 m: obey original mask_camera.
#   - Free voxels: obey original mask_camera (unchanged).
#
# FusionOcc uses mask_camera as a per-voxel binary weight in loss_single,
# so condition_C is implemented by forcing mask_camera=1 for near occupied
...
# Condition C: occupied voxels within 35 m force mask_camera=1 (always supervised);
# free voxels and far occupied voxels obey original mask_camera.
train_pipeline = [
    dict(
...
work_dir = './work_dirs/fusion_occ_occ3d_miou_unified_condition_C'
