...
#
# fusion_occ_occ3d_rayiou_ori_setting.py
#
# Based on fusion_occ_occ3d_miou_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_mask=True  → model.use_mask=True → 카메라 가시 voxel에만 loss 계산
#   - Evaluation: use_image_mask=False → RayIoU는 ray casting으로 가시성 처리, camera mask 미사용
#                 use_mask_eval=False  → val/test 데이터셋에서 GT에 마스킹 없이 원본 voxel label 유지
#
...
use_mask = True       # 학습 시 camera mask 사용 (train dataloader, model)
...
work_dir = './work_dirs/fusion_occ_occ3d_rayiou_ori_setting'
