...
# -----------------------------------------------------------------------
# Ablation: condition_C_20m (occupied-only distance mask, 20 m threshold)
#   - Occupied voxels < 20 m from ego: always supervised (force mask_camera=1)
#     regardless of camera visibility.
#   - Occupied voxels >= 20 m: obey original mask_camera.
#   - Free voxels: obey original mask_camera (unchanged).
#
# FusionOcc uses mask_camera as a per-voxel binary weight in loss_single,
# so condition_C_20m is implemented by forcing mask_camera=1 for near occupied
# voxels in LoadOccGTFromFile (rather than setting voxel_semantics to 255).
# Eval: unchanged (evaluator loads mask_camera independently).
#
# Comparison vs condition_C (35 m threshold):
...
# condition_C_20m: occupied voxels within 20 m force mask_camera=1 (always supervised);
# free voxels and far occupied voxels (>= 20 m) obey original mask_camera.
train_pipeline = [
    dict(
...
work_dir = './work_dirs/fusion_occ_occ3d_miou_unified_condition_C_20m'
