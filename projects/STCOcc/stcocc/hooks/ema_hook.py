...
__all__ = ['ModelEMA', 'MEGVIIEMAHook']
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, updates=0):
        ...
        # Handle different model structures properly
        if is_parallel(self.ema_model):
            self.ema = self.ema_model.module
        else:
...
class MEGVIIEMAHook(Hook):
    ...
    def before_run(self, runner):
        ...
        if self.resume is not None:
            ...
            # Use mmengine's load_checkpoint instead of load_state_dict
            load_checkpoint(runner.ema_model.ema, cpt['state_dict'])
            runner.ema_model.updates = cpt['updates']

    def after_train_iter(self, runner, batch_idx=None, data_batch=None, outputs=None):
        # Get the actual model (handle both parallel and non-parallel cases)
        model = runner.model.module if hasattr(runner.model, 'module') else runner.model
        runner.ema_model.update(runner, model)
