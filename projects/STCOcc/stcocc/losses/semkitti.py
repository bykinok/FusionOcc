    ...
    if isinstance(ssc_target, list):
        ...
        for t in ssc_target:
            if isinstance(t, np.ndarray):
                target_tensors.append(torch.from_numpy(t))
            elif torch.is_tensor(t):
                target_tensors.append(t)
            else:
                target_tensors.append(torch.tensor(t))
...
def sem_scal_loss(pred_, ssc_target, ignore_index=255):
    ...
    # Ensure ssc_target is on the same device as pred_
    if hasattr(pred_, 'device'):
        ssc_target = ssc_target.to(pred_.device)
