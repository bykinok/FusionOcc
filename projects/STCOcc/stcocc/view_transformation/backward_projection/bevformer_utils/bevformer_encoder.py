    ...
    def digit_version(version_str: str):
        ...
        for x in version_str.split('.'):
            if x.isdigit():
                ...
                digit_ver.append(int(patch_version[0]))
...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    # @force_fp32  # deprecated decorator(apply_to=('reference_points', 'cam_params'))
    def point_sampling(self, reference_points, pc_range, img_metas, cam_params=None):
        ...
        if ego2lidar.dim() == 5 and ego2lidar.size(2) > 1:
            # If ego2lidar has shape [1, B, N, 4, 4], take the first transformation for each batch
            ego2lidar = ego2lidar[:, :, 0:1]  # Keep shape [1, B, 1, 4, 4]
