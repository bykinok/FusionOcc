...
try:
    ...
    # Fallback: bev_pool_v2 not available in this mmdet3d version
    bev_pool_v2 = None
    print("Warning: bev_pool_v2 not available, using fallback implementation")
...
class LSSForwardProjection(LSSViewTransformer):
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        Output:
            gt_depths: [B*N*h*w, d]
        """
        # Handle case where gt_depths is a list
        if isinstance(gt_depths, list):
            ...
            for d in gt_depths:
                if isinstance(d, np.ndarray):
                    ...
                    depth_tensors.append(torch.tensor(d))
            gt_depths = torch.stack(depth_tensors)
