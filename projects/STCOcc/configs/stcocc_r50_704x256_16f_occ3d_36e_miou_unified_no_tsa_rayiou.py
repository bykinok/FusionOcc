...
# 원본: stcocc_r50_704x256_16f_occ3d_36e_miou_unified.py
# 변경사항 (backward_projection.transformer.encoder):
#   - use_temporal=False (원본: True)
#   - OA_TemporalAttention 제거 (attn_cfgs에서 삭제)
#   - operation_order에서 'self_attn', 'norm' 제거
#     (원본: ('predictor', 'self_attn', 'norm', 'cross_attn', 'norm', 'conv')
#      변경: ('predictor', 'cross_attn', 'norm', 'conv'))
    ...
    backward_projection=dict(
        ...
        transformer=dict(
            ...
            encoder=dict(
                ...
                num_layers=2,
                use_temporal=False,  # TSA 비활성화 (원본: True)
                pc_range=point_cloud_range,
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        # TSA 비활성화: OA_TemporalAttention 제거
                        # dict(
                    ...
                    # TSA 비활성화: 'self_attn', 'norm' 제거
                    # 원본: ('predictor', 'self_attn', 'norm', 'cross_attn', 'norm', 'conv')
                    operation_order=('predictor', 'cross_attn', 'norm', 'conv')
                    )
                ),
        ),
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        work_dir='stcocc_r50_704x256_16f_occ3d_36e_no_tsa',
        eval_show=True))
