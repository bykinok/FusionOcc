...
#     camera visibility, at every distance from ego.
#   - Free voxels: obey original mask_camera (unchanged).
# Eval: unchanged (OccupancyMetric uses mask_camera independently).
#
# Comparison table (occupied voxel supervision):
#   condition_C_20m  → occupied supervised 0–20 m only
#   condition_C      → occupied supervised 0–35 m only
#   condition_C_full → occupied supervised 0–∞  (this config)
# -----------------------------------------------------------------------
...
# condition_C_full: ALL occupied voxels are always supervised regardless of distance;
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        work_dir='stcocc_r50_704x256_16f_occ3d_36e_condition_C_full',
        eval_show=True))
