...
#
# stcocc_r50_704x256_16f_occ3d_36e_miou_wo_train_cam_mask_unified_wo_DepthSV.py
#
# Based on stcocc_r50_704x256_16f_occ3d_36e_miou_wo_train_cam_mask_unified.py
#
# Camera mask policy:
#   - Training  : ignore_invisible=False → 비가시 voxel GT를 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_image_mask=True → mIoU는 camera mask 적용
#                 eval_metric='miou'
#
# wo_DepthSV: loss_depth_weight=0.0 → depth supervision 제거
#
...
# 학습 시 camera mask 미사용: ignore_invisible=False
train_pipeline = [
...
# 평가 시 camera mask 사용: ignore_invisible=True 유지
test_pipeline = [
...
train_dataloader = dict(
    ...
    batch_sampler=dict(
        ...
        batch_size=samples_per_gpu,  # 실제 배치 크기
        world_size=None,
        rank=None,
        seed=0
    ),
    dataset=dict(
        ...
        work_dir='stcocc_r50_704x256_16f_occ3d_36e_wo_train_cam_mask_wo_DepthSV',
        eval_show=True))
...
train_cfg = dict(
    ...
    val_interval=99999)  # No validation
...
# EMA 비활성 (BEVFormer/SurroundOcc와 동일)
custom_hooks = []
...
# Reproducibility: seed=0. EMA 비활성(custom_hooks). SyncBN/AMP 미사용(OptimWrapper, BN)
