...
#
# tpvformer_8xb1-2x_nus-occ3d_wo_train_cam_mask.py
#
# Based on tpvformer_8xb1-2x_nus-occ3d_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_camera_mask_train=False → LoadOccupancy가 비가시 voxel을 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_camera_mask=True → val_pipeline과 evaluator에서 camera mask 사용
#                 → 카메라 가시 voxel만 mIoU 계산
#
...
use_camera_mask = True         # 평가용: camera mask 사용 → 카메라 가시 voxel만 mIoU 계산
...
val_pipeline = [
    ...
    dict(
        ...
        use_occ3d=True,
        pc_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4],
        use_camera_mask=use_camera_mask),  # 평가용: camera mask 유지
    # 원본과 동일한 정규화 값 적용
    dict(
        type='MultiViewImageNormalize',
        mean=[103.530, 116.280, 123.675],
...
val_evaluator = dict(
    ...
    use_image_mask=use_camera_mask if dataset_name == 'occ3d' else False,  # 평가 시 camera mask 사용 → 카메라 가시 voxel만 mIoU 계산
