...
#
# tpvformer_8xb1-2x_nus-occ3d_ori_setting_rayiou.py
#
# Based on tpvformer_8xb1-2x_nus-occ3d_ori_setting.py
#
# Evaluation metric: RayIoU (LiDAR ray 기반)
#   - Training  : use_camera_mask=True → LoadOccupancy가 비가시 voxel을 255로 마스킹
#   - Evaluation: use_camera_mask=False → GT voxel 전체 사용 (RayIoU는 camera mask 불필요)
#                 use_image_mask=False → evaluator에서 camera mask 미적용
#
...
# Camera mask 설정: 학습 시 적용, 평가(RayIoU) 시 미적용
use_camera_mask = True  # 학습용: camera mask 적용 (invisible voxels → 255)
...
val_pipeline = [
    ...
    dict(
        ...
        use_occ3d=True,
        pc_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4],
        use_camera_mask=False),  # RayIoU: camera mask 미적용 → GT voxel 전체 사용
    # 원본과 동일한 정규화 값 적용
    dict(
        type='MultiViewImageNormalize',
        mean=[103.530, 116.280, 123.675],
