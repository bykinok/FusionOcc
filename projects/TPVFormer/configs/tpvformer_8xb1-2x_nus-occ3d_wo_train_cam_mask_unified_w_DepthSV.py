...
# Camera mask 설정 (학습/평가 분리)
use_camera_mask_train = False  # 학습 시 camera mask 미사용
use_camera_mask_eval = True   # 평가 시 camera mask 사용
...
train_pipeline = [
    ...
    dict(
        ...
        use_camera_mask=use_camera_mask_train),  # 학습 시 camera mask 미사용
...
val_pipeline = [
    ...
    dict(
        ...
        use_occ3d=True,
        pc_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4],
        use_camera_mask=use_camera_mask_eval),  # 평가 시 camera mask 사용
...
# 평가 시 camera mask 사용 (use_image_mask=use_camera_mask_eval)
val_evaluator = dict(
    type='OccupancyMetricHybrid',  # 하이브리드 메트릭 사용 (occ3d 및 기존 GT 모두 지원)
    dataset_name=dataset_name,  # Config에서 전달
    num_classes=18,
    use_lidar_mask=False,
    use_image_mask=use_camera_mask_eval if dataset_name == 'occ3d' else False,  # 평가 시 camera mask 사용
