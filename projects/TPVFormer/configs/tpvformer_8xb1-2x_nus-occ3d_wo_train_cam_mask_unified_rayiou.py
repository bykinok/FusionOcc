...
#
# tpvformer_8xb1-2x_nus-occ3d_wo_train_cam_mask_unified_rayiou.py
#
# Based on tpvformer_8xb1-2x_nus-occ3d_wo_train_cam_mask_unified.py
#
# Camera mask policy:
#   - Training  : use_camera_mask_train=False → LoadOccupancy가 비가시 voxel을 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_camera_mask_eval=False → RayIoU는 camera mask 불필요
...
use_camera_mask_eval = False   # 평가 시 camera mask 미사용 (RayIoU 기반)
...
val_pipeline = [
    ...
    dict(
        ...
        use_occ3d=True,
        pc_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4],
        use_camera_mask=use_camera_mask_eval),  # RayIoU: camera mask 미적용 → GT voxel 전체 사용
