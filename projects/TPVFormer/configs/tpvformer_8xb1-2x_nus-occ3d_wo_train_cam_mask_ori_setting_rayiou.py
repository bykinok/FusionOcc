...
#
# tpvformer_8xb1-2x_nus-occ3d_wo_train_cam_mask_ori_setting_rayiou.py
#
# Based on tpvformer_8xb1-2x_nus-occ3d_wo_train_cam_mask_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_camera_mask_train=False → LoadOccupancy가 비가시 voxel을 255로 마스킹하지 않음
#                 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_camera_mask=False → RayIoU는 camera mask 불필요
#                 use_image_mask=False → evaluator에서 camera mask 미적용 (RayIoU 기반)
#
...
use_camera_mask_train = False  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 계산
use_camera_mask = False        # 평가용: RayIoU → camera mask 미적용
...
train_pipeline = [
    ...
    dict(
        ...
        use_camera_mask=use_camera_mask_train),  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 계산
...
# RayIoU 평가: camera mask 미적용 (GT voxel 전체 사용)
val_pipeline = [
    dict(
        type='BEVLoadMultiViewImageFromFiles',
        to_float32=True,  # 원본과 동일하게 float32로 변환
    ...
    dict(
        ...
        use_occ3d=True,
        pc_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4],
        use_camera_mask=use_camera_mask),  # RayIoU: camera mask 미적용 → GT voxel 전체 사용
    # 원본과 동일한 정규화 값 적용
    dict(
        type='MultiViewImageNormalize',
        mean=[103.530, 116.280, 123.675],
