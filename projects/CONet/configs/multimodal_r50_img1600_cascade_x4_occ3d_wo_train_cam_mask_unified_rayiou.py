...
#
# multimodal_r50_img1600_cascade_x4_occ3d_wo_train_cam_mask_unified_rayiou.py
#
# Based on multimodal_r50_img1600_cascade_x4_occ3d_wo_train_cam_mask_unified.py
#
# Camera mask policy:
#   - Training  : use_camera_mask=False (visible_mask=False) → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_image_mask=False  → RayIoU는 ray casting으로 가시성 처리, camera mask 미사용
#                 use_camera_mask=False (visible_mask_eval=False) → GT에 마스킹 없이 원본 voxel label 유지
#
_base_ = [
...
visible_mask = False        # 학습 시 camera mask 미사용
visible_mask_eval = False   # RayIoU 평가: GT 마스킹 없이 원본 voxel label 유지
...
# 학습 시 camera mask 미사용 (visible_mask=False)
train_pipeline = [
    ...
    dict(type='LoadPointsFromMultiSweeps',
        sweeps_num=10),
    dict(type='PointsLidar2Ego'),  # Transform points to ego coordinate (FusionOcc approach)
    ...
    dict(type='LoadOccupancy', 
         ...
         cal_visible=visible_mask_eval,  # GT 로드 시 visible 계산은 유지
         use_occ3d=use_occ3d,
...
# RayIoU 평가: camera mask 미사용 (visible_mask_eval=False)
test_pipeline = [
    dict(type='LoadPointsFromFile',
        coord_type='LIDAR',
        load_dim=5,
    ...
    dict(type='LoadOccupancy', 
         ...
         unoccupied=empty_idx, 
         pc_range=point_cloud_range, 
         cal_visible=visible_mask_eval,
         use_occ3d=use_occ3d,
         use_camera_mask=visible_mask_eval),  # RayIoU: visible_mask_eval=False → 마스킹 없이 원본 voxel label 유지
...
# Reproducibility: seed=0. EMA/SyncBN(→BN) 비활성. AMP 미사용(OptimWrapper)
