...
#
# multimodal_r50_img1600_cascade_x4_occ3d_wo_train_cam_mask.py
#
# Based on multimodal_r50_img1600_cascade_x4_occ3d_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_camera_mask=False → GT에 255(ignore) 마스킹 없음 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_image_mask=True   → OccupancyMetricHybrid가 카메라 가시 voxel만 mIoU 계산
#
_base_ = [
...
visible_mask = True  # 평가용: Occ3D provides mask_camera → 평가 메트릭에서 사용

# Camera mask policy (학습/평가 분리)
visible_mask_train = False  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 적용
# visible_mask (True)는 test_pipeline과 evaluator에서 사용
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         cal_visible=visible_mask,
         use_occ3d=use_occ3d,
         use_camera_mask=visible_mask),  # 평가 파이프라인은 visible_mask=True 유지
...
val_evaluator = dict(
    ...
    use_image_mask=True,  # 평가 시 camera mask 사용 → 카메라 가시 voxel만 mIoU 계산
