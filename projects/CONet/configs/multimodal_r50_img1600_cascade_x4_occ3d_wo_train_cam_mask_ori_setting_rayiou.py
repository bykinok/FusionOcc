...
#
# multimodal_r50_img1600_cascade_x4_occ3d_wo_train_cam_mask_ori_setting_rayiou.py
#
# Based on multimodal_r50_img1600_cascade_x4_occ3d_wo_train_cam_mask_ori_setting.py
#
# Camera mask policy:
#   - Training  : use_camera_mask=False → GT에 255(ignore) 마스킹 없음 → 전체 voxel에 대해 loss 계산
#   - Evaluation: use_image_mask=False  → RayIoU는 ray casting으로 가시성 처리, camera mask 미사용
#                 use_camera_mask=False → test_pipeline에서 GT에 마스킹 없이 원본 voxel label 유지
#
_base_ = [
...
visible_mask = False  # RayIoU 평가용: test_pipeline에서 GT 마스킹 없이 원본 voxel label 유지

# Camera mask policy (학습/평가 분리)
visible_mask_train = False  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 적용
# RayIoU: visible_mask=False → test_pipeline에서도 camera mask 미적용
...
train_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         use_occ3d=use_occ3d,
         use_camera_mask=visible_mask_train),  # 학습 시 camera mask 미사용 → 전체 voxel에 loss 계산
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', 
         ...
         cal_visible=visible_mask,
         use_occ3d=use_occ3d,
         use_camera_mask=visible_mask),  # RayIoU: visible_mask=False → 마스킹 없이 원본 voxel label 유지
