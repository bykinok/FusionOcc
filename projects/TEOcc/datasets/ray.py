...
def generate_rays(coors, label_depths, label_segs, c2w, intrins, max_ray_nums=0, time_ids=None, dynamic_class=None, balance_weight=None, weight_adj=0.3, weight_dyn=0.0, use_wrs=True):
    ...
    if not use_wrs:
        ...
        for i in range(len(rays)):
            # wrs-a
            adj_weight = weight_adj if ids[i] != 0 else 1  
            time_weight = torch.ones(rays[i].shape[0]) * adj_weight
            
            # wrs-c 
            class_weight = balance_weight.clone()
            for class_idx in range(17):
                class_weight[class_idx] = balance_weight[class_idx] if (rays[i][:,3] == class_idx).sum()>0 else 0   
            ray_weight = class_weight[rays[i][:,3].long()]
            
            # wrs-d
            dynamic_weight = torch.zeros_like(ray_weight)
            for class_idx in dynamic_class:
                dynamic_weight[rays[i][:,3].long() == class_idx] = weight_dyn

            ray_weight += dynamic_weight
            weights.append(time_weight * ray_weight)
        
        # sample rays using WRS
        weights = torch.cat(weights)
        rays = torch.cat(rays, dim=0)
        if max_ray_nums > 0 and len(rays) > max_ray_nums:
            sampler = WeightedRandomSampler(weights, max_ray_nums)
...
def generate_rays_nframe(coors, label_depths, label_segs, c2w, intrins, imgs, max_ray_nums=0, time_ids=None):
    """
    multi-frame multi-cam rays
    """
    # generate rays
    rays = []
    for time_id in time_ids:
        for i in time_ids[time_id]:
            ray = pts2ray(coors[i], label_depths[i], label_segs[i], c2w[i], intrins[i])
            ray = torch.cat([ray, imgs[i]], dim=-1) # add image rgb
            rays.append(ray)
    ...
    # random sample
    if max_ray_nums > 0 and len(rays) > max_ray_nums:
        rays = rays[torch.randperm(len(rays))[:max_ray_nums]]
