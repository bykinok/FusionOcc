...
from ..compat import get_dist_info
...
try:
    ...
    from .samplers.group_sampler import DistributedGroupSampler as GroupSampler
...
try:
    ...
    from torch.utils.data import ConcatDataset as _concat_dataset
...
def custom_build_dataset(cfg, default_args=None):
    try:
        from torch.utils.data import ConcatDataset, Subset as RepeatDataset
    ...
    elif cfg['type'] == 'CBGSDataset' and CBGSDataset is not None:
        dataset = CBGSDataset(custom_build_dataset(cfg['dataset'], default_args))
    else:
        # 새 mmdet3d API 우선, 폴백으로 구버전
        try:
            from mmdet3d.registry import DATASETS as _D
            dataset = _D.build(cfg)
        except Exception:
