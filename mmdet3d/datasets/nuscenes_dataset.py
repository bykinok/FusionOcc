...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
                 ...
                 start_idx=0,
                 multi_adj_frame_id_cfg_lidar=None,
                 use_mask=True,
                 ego_cam='CAM_FRONT',
                 stereo=False):
        ...
        self.multi_adj_frame_id_cfg_lidar = multi_adj_frame_id_cfg_lidar
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                if 'fusion' in self.img_info_prototype:
                    info_adj_list = self.get_adj_info(info, index)
                    input_dict.update(dict(adjacent=info_adj_list))
                    info_adj_list_lidar = self.get_adj_info_lidar(info, index)
                    input_dict.update(dict(lidar_adjacent=info_adj_list_lidar))
        return input_dict

    def get_adj_info_lidar(self, info, index):
        info_adj_list = []
        adj_id_list = list(range(*self.multi_adj_frame_id_cfg_lidar))
        if self.stereo:
            assert self.multi_adj_frame_id_cfg_lidar[0] == 1
            assert self.multi_adj_frame_id_cfg_lidar[2] == 1
            adj_id_list.append(self.multi_adj_frame_id_cfg_lidar[1])
